...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent) {
  ...
  qDebug() << "Tentative de chargement du style depuis:"
           << styleFile.fileName();
...
void MainWindow::createHomePage() {
  ...
  titleLabel->setStyleSheet(
      "color: white; font-size: 36px; font-weight: bold; margin-bottom: 20px;");
...
void MainWindow::createPages() {
  ...
  connect(settingsPage, &Settings::requestNavigateToPage, this,
  ...
  connect(pentestPage, &Pentest::requestNavigateToPage, this,
          &MainWindow::navigateToPage);

  NmapScanner *nmapScanner = pentestPage->findChild<NmapScanner *>();
  if (nmapScanner) {
    connect(nmapScanner, &NmapScanner::scanCompleted, this,
            &MainWindow::showScanResults);
  }
  ...
  connect(resultPage, &ScanResultPage::requestNavigateToPage, this,
          &MainWindow::navigateToPage);
  ...
  connect(historiquePage, &Historique::requestNavigateToPage, this,
          &MainWindow::navigateToPage);
}
...
QPushButton *MainWindow::createMenuButton(const QString &text,
  ...
  iconLabel->setPixmap(QPixmap(iconPath).scaled(32, 32, Qt::KeepAspectRatio,
  ...
  connect(button, &QPushButton::clicked, this,
          [this, pageName]() { navigateToPage(pageName); });
 ...
 */
void MainWindow::showScanResults(const QString &target, const QString &output,
                                 bool success) {
  ScanResultPage *resultPage =
      qobject_cast<ScanResultPage *>(m_pages.value("ScanResults"));
  if (resultPage) {
